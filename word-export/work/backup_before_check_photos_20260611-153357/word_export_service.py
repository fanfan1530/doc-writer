...
def build_check_record_docx(content: str) -> bytes:
...
def build_docx(content: str, doc_type: str = "") -> bytes:
    if "检查笔录" in str(doc_type or "") and CHECK_TEMPLATE_PATH.exists():
        try:
            return build_check_record_docx(content)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        file_path.write_bytes(build_docx(content, doc_type))
