...
def _birth_from_id(id_number: str):
    text = re.sub(r'\s+', '', str(id_number or ''))
    match = re.match(r'^\d{6}(\d{4})(\d{2})(\d{2})', text)
    if match:
        year, month, day = (int(part) for part in match.groups())
        try:
            return date(year, month, day)
...
def _format_chinese_date(value, default_today: bool = False) -> str:
    parsed = _parse_date(value)
    if not parsed and default_today:
        parsed = date.today()
    if parsed:
...
def _normalize_violation(value) -> str:
    ...
    text = text.strip('：:;；，,。 ')
    if text in ('无', '不明', '未知', '行政案件', '治安案件', '行政处罚决定书', '检查笔录', '辨认笔录', '入所表', '前科证明'):
        return ''
    return text[:40]
...
def _build_criminal_record_certificate(elements: dict) -> str:
    name = _first(elements.get('party_name'), elements.get('person_name'), elements.get('checked_person_name'), elements.get('applicant_name'))
    gender = _first(elements.get('party_gender'), elements.get('person_gender'), elements.get('gender'))
    id_type = _first(elements.get('id_type'), elements.get('certificate_type')) or '居民身份证'
    id_number = _first(elements.get('party_id_number'), elements.get('id_number'), elements.get('person_id_number'))
    ...
    household_address = _first(
        elements.get('household_address'),
        elements.get('household_register_detail'),
        elements.get('household_register'),
        elements.get('party_address'),
        elements.get('address'),
    )
    agency = _first(elements.get('issuing_agency'), elements.get('police_station'), elements.get('handling_unit')) or '广西藤县公安局埌南派出所'
    issue_date = _format_chinese_date(_first(elements.get('issue_date'), elements.get('stamp_date')), default_today=True)
    record_result = _first(elements.get('criminal_record_result'), elements.get('criminal_record')) or '未发现'
    if '有' in record_result and '无' not in record_result and '未发现' not in record_result:
        result_line = f'经我所民警在全国违法犯罪人员信息资源库、全国在逃人员信息库、全国吸毒人员信息库、广西警务信息综合应用平台和我所档案室查找档案，在本证明出具之日前，发现{name}在本辖区居住期间有违法犯罪记录。'
    else:
        result_line = f'经我所民警在全国违法犯罪人员信息资源库、全国在逃人员信息库、全国吸毒人员信息库、广西警务信息综合应用平台和我所档案室查找档案，在本证明出具之日前，未发现{name}在本辖区居住期间有违法犯罪记录。'

    return '\n\n'.join([
        '前科证明',
        f'{name}，性别：{gender} ，出生日期：{birth}，                  身份证件种类及号码：{id_type}{id_number}，户籍住址：{household_address}。',
        result_line,
        '特此证明。',
        agency,
        issue_date,
...
def main(
    ...
    if doc_type == '入所表':
        ...
        content = _build_criminal_record_certificate(elements)
    elif doc_type == '检查笔录':
    ...
    output_content = content.strip() if doc_type in ('检查笔录', '辨认笔录', '入所表', '前科证明') else _clean_content(content)
    return {
...
def main(input_text: str, doc_type: str = '', workflow_run_id: str = '', elements_json: str = '') -> dict:
    ...
    if '入所' in doc_type:
        ...
        filename = 'generated-criminal-record-certificate.docx'
    elif '辨认' in doc_type:
...
variables.insert(0, {
    ...
    "options": ["行政处罚决定书", "检查笔录", "辨认笔录", "入所表", "前科证明"],
    "max_length": 48,
    "placeholder": "请选择文书类型",
    "hint": "选择本次要生成的公安法律文书",
})
...
要求：
1. 如果用户选择了“行政处罚决定书”“检查笔录”“辨认笔录”“入所表”或“前科证明”，doc_type必须以用户选择为准。
...
如果目标文书是前科证明：
- 只提取证明对象的姓名、性别、出生日期、身份证件类型及号码、户籍住址、出具单位、出具日期、是否发现违法犯罪记录。
- 不要生成行政处罚事实段、检查过程、辨认过程或入所材料。
...
{"elements": {
  ...
  "issuing_agency":"前科证明出具单位",
  "issue_date":"前科证明出具日期",
  "criminal_record_result":"是否发现违法犯罪记录"
}, "suggested_laws": ["法律条文"]}"""
...
如果目标文书是前科证明：
1. 只输出“未发现某人在本辖区居住期间有违法犯罪记录”这一类证明结果句。
2. 不要写行政处罚事实、法律适用、检查过程、辨认过程或入所材料。
