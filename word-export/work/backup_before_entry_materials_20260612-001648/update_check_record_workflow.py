...
def _normalize_violation(value) -> str:
    ...
    text = text.strip('：:;；，,。 ')
    if text in ('无', '不明', '未知', '行政案件', '治安案件', '行政处罚决定书', '检查笔录', '辨认笔录'):
        return ''
    return text[:40]
...
def main(
    ...
    output_content = content.strip() if doc_type in ('检查笔录', '辨认笔录') else _clean_content(content)
    return {
...
def main(input_text: str, doc_type: str = '', workflow_run_id: str = '') -> dict:
...
要求：
1. 如果用户选择了“行政处罚决定书”“检查笔录”或“辨认笔录”，doc_type必须以用户选择为准。
