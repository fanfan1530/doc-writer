...
@router.post("/delete")
async def delete_model(body: DeleteModelRequest):
    """删除指定模型配置。"""
    try:
        result = model_manager.delete_model(body.model_id)
...
async def test_connection(body: TestConnectionRequest):
    """测试模型连接是否正常（使用请求中提供的 API Key）。"""
    result = await model_manager.test_connection(body.model_dump())
    return result


@router.post("/test-by-id")
async def test_connection_by_id(body: SwitchModelRequest):
    """通过模型 ID 测试连接（使用已存储的 API Key）。"""
    try:
        result = await model_manager.test_connection_by_id(body.model_id)
        return result
    except ValueError as e:
        raise HTTPException(status_code=422, detail=str(e))
