...
class FieldProcessor:
    ...
    def _process_inspection_record(
        ...
        if items.startswith("身上"):
            items = items[2:]
        if not items or items in ("null", "none", "无", "有", "是", "否", "未发现", "未", "发现", ""):
        ...
        if injury.startswith("身上"):
            injury = injury[2:]
        if not injury or injury in ("null", "none", "无", "有", "是", "否", "未发现", "未", "发现", ""):
    ...
    def _process_penalty_decision(
            ...
            if law_result:
                ...
                if not fields.get("penalty_content"):
                    ...
                    if fine and fine not in ("null", "none", "", "0"):
                        ...
                        fields["penalty_content"] = law_result[2]
        ...
        if not fields.get("doc_number"):
            ...
            if cn:
                ...
                year_match = re.search(r"(\d{4})", fields.get("penalty_decision_date", ""))
