...
10. 辨认笔录交叉映射规则（输入来自询问/讯问笔录摘要时）：
   ...
   - "案发地点"→ case_location
11. **处罚依据提取规则（重要）**：
   - penalty_basis 必须从输入文本中提取或根据案由合理推断
   - 对于行政案件，优先引用《中华人民共和国治安管理处罚法》的实体性条款（第23-76条）
   - 严禁引用程序性条款（第82条传唤、第83条询问时限、第87条检查、第89条扣押等）作为处罚依据
   - 严禁引用《中华人民共和国刑事诉讼法》（辨认/搜查/扣押条款）作为行政处罚依据
   - 如果输入文本未明确提及具体处罚条款，根据案由选择最匹配的实体罚则
12. **illegal_fact 字段规则**：
   - 直接输出案件事实描述，不要加"现查明："、"现查明"、"经查："等前缀
   - 不要包含"以上事实有XX证据证实"等证据列举句，证据由模板统一追加
13. **检查笔录 items_found/injury_found 规则**：
   - items_found 应填"发现手机一部"或"没有发现任何涉案物品"等实质性描述，不要填"是，X"
   - injury_found 应填"无任何损伤"或具体损伤描述，不要填"是"或"否"
"""
...
TIMELINE_EXTRACTION_PROMPT = """你是一名资深公安法制审核专家。请从以下案件描述中提取所有带时间信息的事件，按时间顺序排列，输出 JSON 数组。
